...
let ticketPriceTotal = runners.reduce((acc, elem) => (acc+=elem.donation), 0);
...
// Problem 1
// The event director needs to have the list of unique first and last names of runners for the sponsor's marketing department. Return the object having the sorted list of unique first names and the second list of unique last names.
let uniqueFirstAndLastNames = {first_names: runners.map(elem => elem.first_name).filter((val, i, arr) => arr.indexOf(val)===i).sort(), last_names: runners.map(elem => elem.last_name).filter((val, i, arr) => arr.indexOf(val)===i).sort()};
console.log(uniqueFirstAndLastNames);

// Problem 2
// The event director got an info from the bank that the list of donations is wrong. Bank representative said that all the people who donated over $200, should have additional 100 counted towards their donations. Return the sum of the donations.
let sumOfDonations = runners.map(elem => elem.donation>200 ? elem.donation+100 : elem.donation).reduce((acc, val) => acc+=val, 0);
console.log(sumOfDonations);

// Problem 3
// The sponsor requires the event director to deliver the sum of donations value per shirt size. Return an object containing these informations.
let companies = ((arr) => {
    return {
        "XS": arr.filter(elem => elem.shirt_size==="XS").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0),
        "S": arr.filter(elem => elem.shirt_size==="S").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0),
        "M": arr.filter(elem => elem.shirt_size==="M").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0),
        "L": arr.filter(elem => elem.shirt_size==="L").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0),
        "XL": arr.filter(elem => elem.shirt_size==="XL").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0),
        "2XL": arr.filter(elem => elem.shirt_size==="2XL").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0),
        "3XL": arr.filter(elem => elem.shirt_size==="3XL").map(elem => elem.donation).reduce((acc, val) => acc+=val, 0)
    };
})(runners);
